...
export default class BiPspbActiveChallenges extends LightningElement {
	...
	renderedCallback() {
		...
		try {
			...
			this.compqurl = this.urlq + this.biPspbWapiCompletedQuestionnaire;
			this.incomqurl = this.urlq + this.brWapiQuestionnaire;
			console.log('incomqurl:', this.incomqurl);
	...
	updateNewVar() {
		if(this.stwai > 0)
		{
			console.log("Number of Assessments in if : " + this.stwai, this.trackYourGppDivWpai);
			this.newvar = true;
			console.log('this.newvar true', this.newvar);

			
		}
		else
		{
			console.log("Number of Assessments in else : " + this.stwai, this.trackYourGppDivWpai);
			this.newvar = false;
			console.log('this.newvar false', this.newvar);
		}
    }
